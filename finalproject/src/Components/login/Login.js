...
export const Login = () => {
            ...
            .then(response => {
                if(response.data.user){
                    dispatch(login({
                        ...
                        friendAmitier: response.data.friendAmitier,
                        friendAmrx: response.data.friendAmrx
                    }))
                    setUrl(!url)
                }else{
                    dispatch(login({
    ...
    return (
        <div>
            ...
            {err==""? "": <div style={{fontSize:"16px", padding:"0", marginBottom:"20px", display:"flex", justifyContent:"space-evenly", alignItems:"center"}} className="alert alert-danger"><p style={{paddingTop:"10px",}}>remplissez tous les champs s'il vous plait</p><FontAwesomeIcon onClick={()=>setErr("")} style={{cursor:"pointer", margin: "5px"}} icon={faTimesCircle}></FontAwesomeIcon></div>}
            <form action="" className="form-group form-app">
                ...
                <Link to='/resetpassword' style={{fontSize:"16px", color:"#fff", textDecoration:"none", textAlign: "center"}}>mot de passe oublié?</Link>
                <Link to='/dashboard'>
